...
\Fn{\FRec{eksperimentalniSpektar}}{
    ...
    $ciljnaMasa \gets$ Poslednji element liste $eksperimentalniSpektar$\;
    
    \While{$peptidi \neq$ Prazno}{
        $prosireni \gets$ \textbf{ProdužiPeptide}($peptidi$)\;
...
\Fn{\FRec{eksperimentalniSpektar}}{
    $peptidi \gets$ Lista sa praznim stringom\;
    $leaderPeptid \gets$ Prazan peptid\;
    $najboljiRezultat \gets 0$\;
    $ciljnaMasa \gets$ Poslednji element liste $eksperimentalniSpektar$\;
    ...
    \While{$peptidi \neq$ Prazno}{
        ...
        \ForEach{$peptid \in prosireni$}{
            $masa \gets$ \textbf{IzračunajMasu}($peptid$)\;
            \uIf{$masa = ciljnaMasa$}{
                $rezultatKandidata \gets$ \textbf{CikličniScore}($peptid$, $eksperimentalniSpektar$)
                
                \If{$rezultatKandidata > najboljiRezultat$}{
                    $leaderPeptid \gets peptid$\;
                    $najboljiRezultat \gets rezultat$\;
                }
            }\ElseIf{$masa < ciljnaMasa$}{
                Dodaj $peptid$ u $kandidati$\;
            }
...
\SetKwProg{Fn}{Funkcija}{}{}
\Fn{\FRec{eksperimentalniSpektar, S}}{
    $matricaKonvolucije \gets$ Prazna lista\;
    $n \gets$ Broj elemenata u listi $eksperimentalniSpektar$\;

    \For{$i \gets 0$ \KwTo $n-1$}{
        \For{$j \gets 0$ \KwTo $i-1$}{
            $masa \gets eksperimentalniSpektar[i] - eksperimentalniSpektar[j]$\;
            \If{$57 \leq masa \leq 200$}{
                Dodaj $masa$ u $matricaKonvolucije$\;
            }
        }
    }

    $brojPojavljivanja \gets$ Prazna mapa\;
    \ForEach{$masa$ u $matricaKonvolucije$}{
        \If{$masa \in brojPojavljivanja$}{
            $brojPojavljivanja[masa] \gets brojPojavljivanja[masa] + 1$\;
        }
        \Else{
            $brojPojavljivanja[masa] \gets 1$\;
        }
    }

    $sortiraneFrekvencije \gets$ \textbf{SortirajOpadajuće}($brojPojavljivanja$)\;
    $najcesceMase \gets$ Uzimamo $S$ (unapred određen ceo broj) najčešćih masa iz $sortiraneFrekvencije$\;
    
    $leaderPeptid \gets$ \textbf{LeaderboardSequencing}($eksperimentalniSpektar$, $najcesceMase$)\;
